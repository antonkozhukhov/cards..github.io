...
class ValidateNewCard extends Validate{  //класс ValidateNewCard расширяет функционал базового класса
  buttonValidate(){ //описываем специфичное для конкретного попапа поведение
...
placeForm.addEventListener('submit', function(){  
  event.preventDefault()
  api.postPlace(placeName, placeLink).then((result) => {          // Исправил
